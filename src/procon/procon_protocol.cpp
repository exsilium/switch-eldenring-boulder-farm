...
void Protocol::setImuData() {
  ...
  // Neutral IMU sample. "Neutral" is NOT all-zeros: the host subtracts the
  // origins declared in the user IMU calibration we serve from SPI bank
  // 0x8026 (accel origin (-66, 62, 496), gyro origin (-2, -2, +8)), so the
  // raw sample must equal those origins to decode as zero motion. Streaming
  // raw zeros decoded as a constant (+2, +2, -8) gyro rate -> steady yaw-left
  // camera drift. Accel Z carries origin + 1g (1g = (sens 0x4000 - origin)/4)
  // so the controller reads as resting flat.
  static const uint8_t kImu[36] = {
      0xBE, 0xFF, 0x3E, 0x00, 0x74, 0x11, 0xFE, 0xFF, 0xFE, 0xFF, 0x08, 0x00,
      0xBE, 0xFF, 0x3E, 0x00, 0x74, 0x11, 0xFE, 0xFF, 0xFE, 0xFF, 0x08, 0x00,
      0xBE, 0xFF, 0x3E, 0x00, 0x74, 0x11, 0xFE, 0xFF, 0xFE, 0xFF, 0x08, 0x00};
...
void Protocol::spiRead() {
  ...
  static const uint8_t kSpiRom80[] = {
      ...
      0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
      0xFF, 0xFF, 0xFF, 0xFF,
      // 0x10: user left stick calibration -- NO user cal (magic 0xFF 0xFF).
      // The reference ROM advertised user stick cal (magic 0xB2 0xA1) with the
      // 9 data bytes in the *left* layout for both sticks; decoded with the
      // right stick's center/below/above layout that yields center 0x7FF and
      // an above-center range of 0, so a centred (0x800) stick reads as a full
      // deflection -> constant camera spin. Blanking the magic makes the
      // console fall back to the sane factory calibration in bank 0x60.
      0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
      // 0x1B: user right stick calibration -- NO user cal (magic 0xFF 0xFF).
      0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
const uint8_t* Protocol::buildStreamReport(size_t& outLen) {
  ...
  if (_imuEnabled) {
    // Neutral IMU: raw sample == the calibration origins served from SPI, so
    // the host decodes zero motion -- see setImuData() for the derivation.
    static const uint8_t kImu[36] = {
        0xBE, 0xFF, 0x3E, 0x00, 0x74, 0x11, 0xFE, 0xFF, 0xFE, 0xFF, 0x08, 0x00,
        0xBE, 0xFF, 0x3E, 0x00, 0x74, 0x11, 0xFE, 0xFF, 0xFE, 0xFF, 0x08, 0x00,
        0xBE, 0xFF, 0x3E, 0x00, 0x74, 0x11, 0xFE, 0xFF, 0xFE, 0xFF, 0x08, 0x00};
